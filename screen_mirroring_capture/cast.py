...
def _generate_self_signed_cert(tmpdir: str) -> tuple[str, str]:
    """Generate a self-signed cert+key pair using cryptography library."""
    cert_path = str(Path(tmpdir) / "cast.crt")
    key_path = str(Path(tmpdir) / "cast.key")

    key = rsa.generate_private_key(public_exponent=65537, key_size=2048)

    subject = issuer = x509.Name([x509.NameAttribute(NameOID.COMMON_NAME, "CastReceiver")])
    cert = (
    ...
    def _accept_loop(self) -> None:
        while not self._stop_event.is_set():
            try:
                ...
                # Client disconnected during SSL handshake - normal for Cast protocol
                log.debug("Cast client disconnected during handshake")
                continue
